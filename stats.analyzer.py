...
class ChessStatsAnalyzer:
    ...
    def _load_learning_stats(self):
        ...
        try:
            with open(os.path.join(self.base_dir, "learned_positions.json"), 'r') as f:
    ...
    def _load_elo_stats(self):
        ...
        elo_files = [f for f in os.listdir(self.base_dir) if f.startswith('gameplay_elo_results_') and f.endswith('.json')]
    ...
    def generate_comprehensive_report(self, output_file=None):
        ...
        with open(output_file, 'w') as f:
            ...
            f.write("-" * 30 + "\n")
            learning_stats = self._calculate_learning_stats()
            ...
            f.write("-" * 30 + "\n")
            elo_stats = self._calculate_elo_stats()
            for key, value in elo_stats.items():
                f.write(f"{key}: {value}\n")
            f.write("\n")
            ...
            f.write("-" * 30 + "\n")
            color_stats = self._calculate_color_stats()
    ...
    def _calculate_overall_stats(self) -> Dict[str, Any]:
        ...
        for stats in self.game_stats:
            if 'summary' in stats:
                ...
                total_wins += summary.get('overall', {}).get('wins', 0)
                total_draws += summary.get('overall', {}).get('draws', 0)
                total_losses += summary.get('overall', {}).get('losses', 0)
...
def main():
    ...
    # Generate ELO progress plot
    plot_file = analyzer.plot_elo_progress()
    print(f"ELO progress plot generated: {plot_file}")
